...
//
// pinmux -- compile-check test package.
//
// One test, and it checks nothing. Its only job is to prove the UVM phasing
// runs to completion on top of an elaborated pinmux DUT.
//
...
package pinmux_test_pkg;

  import uvm_pkg::*;
  `include "uvm_macros.svh"

  class pinmux_base_test extends uvm_test;
    `uvm_component_utils(pinmux_base_test)

    virtual tb_clk_if clk_if;

    function new(string name = "pinmux_base_test", uvm_component parent = null);

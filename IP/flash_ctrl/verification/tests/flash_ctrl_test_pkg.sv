...
//
// flash_ctrl -- compile-check test package.
//
// One test, and it checks nothing. Its only job is to prove the UVM phasing
// runs to completion on top of an elaborated flash_ctrl DUT.
//
...
package flash_ctrl_test_pkg;
  ...
  class flash_ctrl_base_test extends uvm_test;
    `uvm_component_utils(flash_ctrl_base_test)

    virtual tb_clk_if clk_if;

    function new(string name = "flash_ctrl_base_test", uvm_component parent = null);

# flash_ctrl — verification (trainee brief)
...
tests/flash_ctrl_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
flash_ctrl_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/flash_ctrl.hjson` with
...
- Do not read `vendor/opentitan/hw/top_earlgrey/ip_autogen/flash_ctrl/dv/` until you have attempted the piece

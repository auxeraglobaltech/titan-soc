# spi_device — verification (trainee brief)
...
tests/spi_device_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
spi_device_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/spi_device.hjson` with
...
- Do not read `vendor/opentitan/hw/ip/spi_device/dv/` until you have attempted the piece

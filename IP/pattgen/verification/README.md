# pattgen — verification (trainee brief)
...
tests/pattgen_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
pattgen_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/pattgen.hjson` with
...
- Do not read `vendor/opentitan/hw/ip/pattgen/dv/` until you have attempted the piece

# aon_timer — verification (trainee brief)
...
tests/aon_timer_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
aon_timer_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/aon_timer.hjson` with
...
- Do not read `vendor/opentitan/hw/ip/aon_timer/dv/` until you have attempted the piece

# rv_timer — verification (trainee brief)
...
tests/rv_timer_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
rv_timer_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/rv_timer.hjson` with
...
- Do not read `vendor/opentitan/hw/ip/rv_timer/dv/` until you have attempted the piece

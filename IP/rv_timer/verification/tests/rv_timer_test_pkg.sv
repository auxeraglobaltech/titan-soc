...
//
// rv_timer -- compile-check test package.
//
// One test, and it checks nothing. Its only job is to prove the UVM phasing
// runs to completion on top of an elaborated rv_timer DUT.
//
...
package rv_timer_test_pkg;
  ...
  class rv_timer_base_test extends uvm_test;
    `uvm_component_utils(rv_timer_base_test)

    virtual tb_clk_if clk_if;

    function new(string name = "rv_timer_base_test", uvm_component parent = null);

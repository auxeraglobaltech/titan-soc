# pwrmgr — verification (trainee brief)
...
tests/pwrmgr_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
pwrmgr_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/pwrmgr.hjson` with
...
- Do not read `vendor/opentitan/hw/top_earlgrey/ip_autogen/pwrmgr/dv/` until you have attempted the piece

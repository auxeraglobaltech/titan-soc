...
//
// rv_dm -- compile-check test package.
//
// One test, and it checks nothing. Its only job is to prove the UVM phasing
// runs to completion on top of an elaborated rv_dm DUT.
//
...
package rv_dm_test_pkg;
  ...
  class rv_dm_base_test extends uvm_test;
    `uvm_component_utils(rv_dm_base_test)

    virtual tb_clk_if clk_if;

    function new(string name = "rv_dm_base_test", uvm_component parent = null);

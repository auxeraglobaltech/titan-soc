# rv_dm — verification (trainee brief)
...
tests/rv_dm_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
rv_dm_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/rv_dm.hjson` with
...
- Do not read `vendor/opentitan/hw/ip/rv_dm/dv/` until you have attempted the piece

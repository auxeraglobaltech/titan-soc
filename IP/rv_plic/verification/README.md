# rv_plic — verification (trainee brief)
...
tests/rv_plic_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
rv_plic_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/rv_plic.hjson` with
...
- Do not read `vendor/opentitan/hw/top_earlgrey/ip_autogen/rv_plic/dv/` until you have attempted the piece

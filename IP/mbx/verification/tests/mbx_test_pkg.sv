...
//
// mbx -- compile-check test package.
//
// One test, and it checks nothing. Its only job is to prove the UVM phasing
// runs to completion on top of an elaborated mbx DUT.
//
...
package mbx_test_pkg;
  ...
  class mbx_base_test extends uvm_test;
    `uvm_component_utils(mbx_base_test)

    virtual tb_clk_if clk_if;

    function new(string name = "mbx_base_test", uvm_component parent = null);

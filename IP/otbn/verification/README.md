# otbn — verification (trainee brief)
...
tests/otbn_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
otbn_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/otbn.hjson` with
...
- Do not read `vendor/opentitan/hw/ip/otbn/dv/` until you have attempted the piece

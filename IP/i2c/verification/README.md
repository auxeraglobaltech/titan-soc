# i2c — verification (trainee brief)
...
tests/i2c_test_pkg.sv    one uvm_test: wait for reset, idle 1000 clks, pass
i2c_tb.f                 the compile filelist
```
...
1. **TL-UL host agent.** Do not write one. Reuse
   ...
   biggest step here.
2. **RAL.** Generate from `../docs/i2c.hjson` with
...
- Do not read `vendor/opentitan/hw/ip/i2c/dv/` until you have attempted the piece

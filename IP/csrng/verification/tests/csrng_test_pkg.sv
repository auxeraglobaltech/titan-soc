...
//
// csrng -- compile-check test package.
//
// One test, and it checks nothing. Its only job is to prove the UVM phasing
// runs to completion on top of an elaborated csrng DUT.
//
...
package csrng_test_pkg;
  ...
  class csrng_base_test extends uvm_test;
    `uvm_component_utils(csrng_base_test)

    virtual tb_clk_if clk_if;

    function new(string name = "csrng_base_test", uvm_component parent = null);
